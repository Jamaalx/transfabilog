...
function getTableNames(provider, useTemp = false) {
  const suffix = useTemp ? '_temp' : '';
  switch (provider) {
    case 'eurowag':
      return {
        transactions: `eurowag${suffix}_transactions`,
        batches: `eurowag${suffix}_import_batches`,
        isTemp: useTemp,
      };
    case 'verag':
      return {
        transactions: `verag${suffix}_transactions`,
        batches: `verag${suffix}_import_batches`,
        isTemp: useTemp,
      };
    case 'dkv':
    default:
      return {
        transactions: `dkv${suffix}_transactions`,
        batches: `dkv${suffix}_import_batches`,
        isTemp: useTemp,
      };
  }
}

// Helper function to get TEMP table names (for import staging)
function getTempTableNames(provider) {
  return getTableNames(provider, true);
}

// Helper function to get FINAL table names (for approved data)
function getFinalTableNames(provider) {
  return getTableNames(provider, false);
}
...
// ============================================================
// TEMPORARY TABLES ENDPOINTS
// These endpoints handle the staging/temp tables before approval
// ============================================================

/**
 * GET /api/v1/dkv/temp/batches
 * List all import batches from TEMP tables (pending approval)
 * Query param: provider=dkv|eurowag|verag
 */
router.get(
  '/temp/batches',
  ...
  async (req, res, next) => {
    try {
      ...
      res.json({
        data: data.map(b => ({ ...b, is_temp: true })),
        pagination: {
          page,
          limit,
          total: count,
 ...
 * List transactions from TEMP tables (pending approval)
 * Query params: provider, batch_id, status, page, limit
 */
router.get(
  '/temp/transactions',
  [
  ...
  async (req, res, next) => {
    try {
      ...
      res.json({
        data: data.map(tx => ({ ...tx, is_temp: true })),
        pagination: {
          page,
          limit,
          total: count,
 ...
 * Approve transactions from TEMP tables:
 * 1. Create expenses in transactions table
 * 2. Move from temp tables to final tables
 * 3. Delete from temp tables
 *
 * Body: { provider, batch_id?, transaction_ids?, create_expenses: true }
 */
router.post(
  '/temp/approve',
  authorize('admin', 'manager'),
  [
    body('provider').isIn(['dkv', 'eurowag', 'verag']),
    body('batch_id').optional().isUUID(),
    body('transaction_ids').optional().isArray(),
    body('create_expenses').optional().isBoolean(),
  ],
  async (req, res, next) => {
    try {
      const errors = validationResult(req);
      ...
      const { provider, batch_id, transaction_ids, create_expenses = true } = req.body;
      const tempTables = getTempTableNames(provider);
      const finalTables = getFinalTableNames(provider);
      ...
      if (batch_id) {
        txQuery = txQuery.eq('batch_id', batch_id);
      }
      ...
      const { data: tempTransactions, error: txError } = await txQuery;

      if (txError) throw txError;

      if (!tempTransactions || tempTransactions.length === 0) {
        return res.status(400).json({
          error: 'No transactions to approve',
          message: 'No pending transactions found in temp tables',
        });
      }

      console.log(`Approving ${tempTransactions.length} transactions from ${provider} temp tables`);
      ...
      for (const tx of tempTransactions) {
        try {
          // 1. Create expense if requested and transaction has truck_id
          if (create_expenses && tx.truck_id) {
            // Determine category and amount based on provider
            let category = 'combustibil';
            let amount = 0;
            let description = '';
            ...
            if (provider === 'dkv') {
              ...
              if (goodsType.includes('toll') || goodsType.includes('taxa') || goodsType.includes('maut')) {
                category = 'taxa_drum';
              }
              amount = tx.payment_value_eur || tx.gross_value_eur || tx.net_purchase_value_eur || 0;
              description = `DKV - ${tx.goods_type || 'Fuel'} - ${tx.station_name || ''} (${tx.country || ''}) - ${tx.quantity || ''}${tx.unit || 'L'}`;
            } else if (provider === 'eurowag') {
              if (tx.service_type === 'TOLL') {
                category = 'taxa_drum';
              }
              amount = tx.gross_amount_eur || tx.net_amount_eur || 0;
              description = `EUROWAG - ${tx.product_type || tx.service_type || 'Fuel'} - ${tx.location || ''} (${tx.country || ''})`;
            } else if (provider === 'verag') {
              const productCat = (tx.product_category || '').toLowerCase();
              if (productCat.includes('toll') || productCat.includes('maut') || productCat.includes('vignette')) {
                category = 'taxa_drum';
              } else if (productCat.includes('fuel') || productCat.includes('diesel') || productCat.includes('adblue')) {
                category = 'combustibil';
              }
              amount = tx.gross_amount || tx.net_amount || 0;
              description = `VERAG - ${tx.product_type || tx.product_category || 'Toll'} - ${tx.country || ''}`;
            }

            // Get transaction date
            const txDate = tx.transaction_time || tx.transaction_date;
            const date = txDate ? txDate.split('T')[0] : new Date().toISOString().split('T')[0];
            ...
            const { data: expense, error: expenseError } = await supabase
              ...
              .insert({
                ...
                created_by: req.user.id,
                vat_amount: tx.vat_amount || tx.vat_amount_eur || 0,
              })
              .select()
              .single();

            if (expenseError) {
              console.error(`Error creating expense for tx ${tx.id}:`, expenseError);
            } else {
              expensesCreated++;
              // Update temp transaction with expense reference
              tx.transaction_id = expense.id;
            }
          }

          // 2. Insert into final table
          const { batch_id: oldBatchId, id: oldId, ...txData } = tx;

          // Remove temp-specific fields and add status
          const finalTxData = {
            ...txData,
            status: 'created_expense',
            notes: tx.notes ? `${tx.notes} | Approved from temp` : 'Approved from temp',
          };

          // We need to handle batch_id - either create a new batch in final table or use existing logic
          // For simplicity, we'll insert without batch_id first
          delete finalTxData.batch_id;
          ...
          if (insertError) {
            console.error(`Error moving tx ${tx.id} to final table:`, insertError);
            results.failed.push({ id: tx.id, error: insertError.message });
            continue;
          }

          // 3. Delete from temp table
          const { error: deleteError } = await supabase
            .from(tempTables.transactions)
            .delete()
            .eq('id', tx.id);
          ...
          if (deleteError) {
            console.error(`Error deleting tx ${tx.id} from temp:`, deleteError);
          }

          transactionsMoved++;
          results.success.push(tx.id);
        } catch (txError) {
          console.error(`Error processing tx ${tx.id}:`, txError);
          results.failed.push({ id: tx.id, error: txError.message });
        }
      }

      // Clean up empty batches from temp table
      if (batch_id) {
      ...
      res.json({
        success: true,
        provider,
        total_processed: tempTransactions.length,
      ...
      console.error('Temp approve error:', error);
      next(error);
    }
  }
);
...
/**
 * DELETE /api/v1/dkv/temp/batch/:batchId
 * Delete a batch and all its transactions from TEMP tables (discard import)
 * Query param: provider=dkv|eurowag|verag
 */
router.delete(
  '/temp/batch/:batchId',
  ...
  async (req, res, next) => {
    try {
      ...
      // First delete all transactions for this batch
      const { error: txDeleteError, count: txCount } = await supabase
        .from(tables.transactions)
        .delete({ count: 'exact' })
        .eq('batch_id', batchId)
        .eq('company_id', req.companyId);
      ...
      if (txDeleteError) {
        console.error('Error deleting temp transactions:', txDeleteError);
      }
      ...
      res.json({
        success: true,
        message: `Deleted temp batch and ${txCount || 0} transactions`,
        batch_id: batchId,
 ...
 * Match a transaction in TEMP table to a truck
 * Query param: provider=dkv|eurowag|verag
 */
router.put(
