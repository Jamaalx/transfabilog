...
/**
 * Import VERAG transactions into TEMP database tables (staging)
 * Data stays in temp until approved via "Creează Cheltuieli"
 * @param {boolean} useTemp - If true, save to temp tables (default: true)
 */
async function importVeragTransactions(fileBuffer, companyId, userId, documentId, fileName, useTemp = true) {
  ...
  const batchTable = useTemp ? 'verag_temp_import_batches' : 'verag_import_batches';
  const txTable = useTemp ? 'verag_temp_transactions' : 'verag_transactions';
  ...
  console.log(`VERAG Import: Using ${useTemp ? 'TEMP' : 'FINAL'} tables`);
