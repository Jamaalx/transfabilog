...
/**
 * Import EUROWAG transactions into TEMP database tables (staging)
 * Data stays in temp until approved via "Creează Cheltuieli"
 * @param {boolean} useTemp - If true, save to temp tables (default: true)
 */
async function importEurowagTransactions(fileBuffer, companyId, userId, documentId, fileName, useTemp = true) {
  ...
  // Determine which tables to use (temp or final)
  const batchTable = useTemp ? 'eurowag_temp_import_batches' : 'eurowag_import_batches';
  const txTable = useTemp ? 'eurowag_temp_transactions' : 'eurowag_transactions';

  // Check for existing transactions in BOTH temp and final tables to prevent duplicates
  const checkTables = useTemp
    ? ['eurowag_temp_transactions', 'eurowag_transactions']
    : ['eurowag_transactions'];
  ...
  for (const tableName of checkTables) {
    ...
    if (existingError) {
      console.warn(`Could not check for existing transactions in ${tableName}:`, existingError.message);
    }
  ...
  console.log(`EUROWAG Import: Using ${useTemp ? 'TEMP' : 'FINAL'} tables`);

  // Create import batch in appropriate table
  ...
  // Batch insert into appropriate table
