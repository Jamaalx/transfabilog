...
/**
 * Import DKV transactions into TEMP database tables (staging)
 ...
 */
async function importDKVTransactions(fileBuffer, companyId, userId, documentId, fileName, mimeType, useTemp = true) {
  ...
  const batchTable = useTemp ? 'dkv_temp_import_batches' : 'dkv_import_batches';
  const txTable = useTemp ? 'dkv_temp_transactions' : 'dkv_transactions';

  // Check for existing transactions in BOTH temp and final tables to prevent duplicates
  const checkTables = useTemp
    ? ['dkv_temp_transactions', 'dkv_transactions']
    : ['dkv_transactions'];
  ...
  console.log(`DKV Import: Using ${useTemp ? 'TEMP' : 'FINAL'} tables`);
